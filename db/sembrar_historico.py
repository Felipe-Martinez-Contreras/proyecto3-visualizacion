"""Siembra de datos históricos sintéticos para la demo del dashboard.

Genera un historial realista de las últimas N horas (default 24) en las 3 tablas
(`Trafico`, `Calidad_Aire_Hist`, `Condiciones_Clima`), con ticks cada M minutos
(default 1). Así los rangos "última hora" y "último día" del dashboard tienen
contenido interesante sin esperar a que el scheduler acumule datos reales.
...
Uso::
    ...
    uv run python -m db.sembrar_historico                    # 24 h, tick cada 1 min
    uv run python -m db.sembrar_historico --horas 6 --paso-minutos 2
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Siembra datos históricos sintéticos en la BD del proyecto.",
    )
    parser.add_argument("--horas", type=float, default=24, help="horas hacia atrás (default: 24)")
    parser.add_argument("--paso-minutos", type=float, default=1, help="minutos entre ticks (default: 1)")
